...
class BinaryTree(gpytorch.Module):
    ...
    def train_tree(self, X, Y, X_idx, nis ,Xbar, to_print=True):
        loss = 0
        if self.root is not None:
            loss = self._train_tree(self.root, X, Y, X_idx, nis, Xbar, to_print)
        return loss

    def _train_tree(self, node, X, Y, X_idx, nis, Xbar, to_print=True):
        ...
        #print('Training GP on classes',node.classes.size(0), len(batch_classes.intersection(node_classes)))
        ...
        if node.classes.size(0) > 1 and len(batch_classes.intersection(node_classes)) > 0:
            # train the left subtree with the full batch nis
            left_loss = self._train_tree(node.left_child, X, Y, X_idx, nis, Xbar, to_print)
            if to_print:
                logging.info('Training GP on classes: ' + str(detach_to_numpy(node.classes).tolist()) + ' ')
            node_X, node_Y, node_idx, node_nis, node_Xbar = self._extract_node_data(node, X, Y, X_idx, nis, Xbar)
            node_loss = node.train_loop(node_X, node_Y, node_idx, node_nis, node_Xbar, to_print)
            # train the right subtree with the full batch nis as well
            right_loss = self._train_tree(node.right_child, X, Y, X_idx, nis, Xbar, to_print)
            loss = left_loss + node_loss + right_loss
        else:
            if to_print:
    ...
    def _extract_node_data(self, node, X, Y, X_idx, nis, Xbar):
        # take data belongs to that node only
        node_X, node_Y_orig, node_Xidx_orig, nis = pytorch_take3(X, Y, X_idx, nis, node.classes)
        ...
        return node_X, node_Y, node_idx, nis, node_Xbar

    def eval_tree_full_path(self, X, nis, num_classes, Xbar, jitter_Kmm=False):
        ...
        while q:
            ...
            # In case more than one class run GP on the node
                ...
                probs = node.model.predictive_posterior(X_star, nis, jitter_Kmm=jitter_Kmm)
