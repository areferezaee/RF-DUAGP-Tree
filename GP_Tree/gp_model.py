...
def comp_derivative(x_train, x_function, K, weights, length_scale):
    
    n_test, n_dims = x_function.size()

    derivative = torch.ones(x_function.size())
    
    for itest in range(n_test):

        term1 = (x_function[itest, :].unsqueeze(0) - x_train).t()
        K = (K[itest, :].unsqueeze(0) - K)
        term2 = K[itest, :] * weights.squeeze()
        #term2 = weights[itest, :].squeeze()
        derivative[itest, :] = term1 @ term2
    #derivative = derivative.to('cuda')
    #if length_scale != None:
    #   derivative *= - 1 / length_scale**2
    return derivative
def _build_variance(x_variance):
        return x_variance
...
class VariationalELBO(gpytorch.Module):
    ...
    def forward(self, K, Y, batch_idx, length_scale, points, nis):
        #print('ma resididm')
        ...
        pointsss = points[self.num_inducing_points:,:]
        ...
        if 1==1:
           ...
           alpha = YKappaMu#*****************for EGP
           
           
           x_cov = torch.empty(len(nis),dtype=K.dtype,device=K.device)
           
           for i in range(len(nis)):
                
                if len(nis[i].size())>1:
                   
                    x_cov[i] =  torch.var(nis[i]) 
                else: 
                   x_cov[i] = nis[i].item()

           derivative = comp_derivative(
                  pointsss, pointsss, 
                  Knn, alpha, length_scale
                    )

           T_derivative = derivative @ (derivative.t()*x_cov.squeeze())
           K_tilde = torch.diagonal(T_derivative + Knn - Knm.matmul(kappa.t())).unsqueeze(1)
        else:
...
class GP_Model(gpytorch.Module):
    def __init__(self,
                 kernel_func,
                 num_classes=2,
                 dtype=torch.float64,
    ...
    def forward_mll(self, Points, nis, Y, batch_idx, to_print):
        ...
        if self.kernel_func == "RBFKernel":
            ...
            mll = self.ELBO(K, Y_n, batch_idx, lengthscale, Points ,nis)
        else:
            mll = self.ELBO(K, Y_n, batch_idx, None, Points ,nis)
        if to_print:
            self.print_hyperparams()
        return mll

    def predictive_posterior(self, Points, nis, jitter_Kmm=False):
        ...
        if self.kernel_func != "LinearKernel":
            
            
            if self.kernel_func == "RBFKernel" or self.kernel_func == "PowRBFKernel":
               lengthscale = self.model.covar_module.base_kernel.lengthscale.squeeze()
            else:
               lengthscale = None
            alpha = mu_s
            
            x_cov = torch.randn(len(nis))                        
            for i in range(len(nis)):
            ...
            derivative = comp_derivative(
                  Pointsss, Pointsss, 
                  Knn, alpha, lengthscale
                    )           
            
            T_derivative = derivative @ (derivative.t()*x_cov.squeeze())#.cuda()
            Sigma_s = torch.diagonal(T_derivative + Knn + Knm.matmul(torch.cholesky_solve(
